...
const main = async () => {
  ...
  const skipIntro = process.env.WEB3SDKIO_CLI_SKIP_INTRO === "true";
...
$$ | $$ | $$ |$$   _____|$$ |  $$ | ____ $$ |      $$ | $$ |  $$ | $$ | $$\\       $$ |
\\$$$$$\\$$$$  |\\$$$$$$$\\  $$$$$$$  |\\$$$$$$$ |\\$$$$$$$ | \\$$$$$$$ | $$ |  $$| $$$$$$$ |
\\_____\\____/  \\_______| \\_______/  \\_______/ \\______/   \\______/  \\__/ \\__/ \\______/ `);
    console.info(`\n 💎 web3sdkio-cli v${cliVersion} 💎\n`);
  }

  program
    .name("web3sdkio-cli")
    .description("Official web3sdkio command line interface")
  ...
  program
    .command("build")
    .description("Compile contract and detect web3sdkio contract extensions")
  ...
  program
    .command("detect")
    .description(
      "(deprecated) Compile contracts and detect implemented web3sdkio contract extensions",
    )
    .option("-p, --path <project-path>", "path to project", ".")
    .option("-d, --debug", "show debug logs")
    .option("-a, --all", "run detection on all contracts")
      ...
      async ({ default: updateNotifier }) => {
        ...
        if (versionInfo.type !== "latest") {
          ...
          if (res.upgrade) {
            ...
            clonedEnvironment.WEB3SDKIO_CLI_SKIP_INTRO = "true";

            const installation = await findPackageInstallation();

            // If the package isn't installed anywhere, just defer to npx web3sdkio@latest
            if (!installation) {
              ...
              await new Promise((done, failed) => {
                const shell = spawn(
                  `npx --yes web3sdkio@latest ${process.argv
                    .slice(2)
                    .join(" ")}`,
                  [],
            ...
            switch (installation.packageManager) {
              case "npm":
                command = installation.isGlobal
                  ? `npm install -g web3sdkio`
                  : `npm install web3sdkio`;
                break;
              case "yarn":
                command = installation.isGlobal
                  ? `yarn global add web3sdkio`
                  : `yarn add web3sdkio`;
                break;
              case "pnpm":
                command = installation.isGlobal
                  ? `pnpm add -g web3sdkio@latest`
                  : `pnpm add web3sdkio@latest`;
                break;
              default:
                console.error(
