...
export async function twCreate(
  ...
  if (!onlyContract) {
    if (!projectPath) {
      const defaultName =
        projectType === "contract" ? "web3sdkio-contracts" : "web3sdkio-app";
      const res = await prompts({
        type: "text",
        name: "path",
    ...
    if (!projectPath) {
      console.log(
        "\nPlease specify the project directory:\n" +
          `  ${chalk.cyan("npx web3sdkio create")} ${chalk.green(
            "<project-directory>",
          )}\n` +
          "For example:\n" +
          `  ${chalk.cyan("npx web3sdkio create")} ${chalk.green(
          ...
          `Run ${chalk.cyan("npx web3sdkio --help")} to see all options.`,
      );
      process.exit(1);
    }
      ...
      if (!contractName) {
        console.log(
          "\nPlease specify the contract name:\n" +
            `  ${chalk.cyan(
              "npx web3sdkio create --contract --name",
