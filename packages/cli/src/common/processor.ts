import { cliVersion, WEB3SDKIO_URL } from "../constants/urls";
...
export async function processProject(
  ...
  if (projectType === "none") {
    if (command === "deploy") {
      info(
        "No contracts detected in this directory. Redirecting to the web3sdkio contract deployment page.",
      );
      return "https://web3sdk.io/contracts";
    }
    ...
    logger.info(``);
    logger.info(`     ${chalk.cyan(`npx web3sdkio@latest create --contract`)}`);
    logger.info(``);
    process.exit(1);
  }
  ...
  const soliditySDKPackage = "@web3sdkio/contracts";
...
export function getUrl(hashes: string[], command: string) {
  ...
  if (hashes.length === 1) {
    ...
    url = new URL(WEB3SDKIO_URL + "/contracts/" + command);
