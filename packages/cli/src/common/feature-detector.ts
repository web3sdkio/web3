...
import { detectFeatures, FeatureWithEnabled } from "@web3sdkio/sdk";
...
export async function detectExtensions(options: any) {
  ...
  let contractsWithFeatures: ContractFeatures[] = selectedContracts.map(
    (contract) => {
      ...
      const enabledFeatures: Feature[] = features.enabledFeatures.map(
        (feature) => ({
          name: feature.name,
          reference: `https://docs.web3sdk.io/interfaces/${feature.name.toLowerCase()}`,
        }),
      );
  ...
  contractsWithFeatures.map((contractWithFeatures) => {
    ...
    let deployCmd = `npx web3sdkio@latest deploy`;
