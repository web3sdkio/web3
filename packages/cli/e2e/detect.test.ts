...
describe("npx web3sdkio detect", () => {
  it("should detect ERC721Base extensions", async () => {
    ...
    expect(await exists("web3sdkio-contracts/contracts/Contract.sol")).toEqual(
      true,
    );

    // check that hardhat config exists
    expect(await exists("web3sdkio-contracts/hardhat.config.js")).toEqual(true);

    await create.writeText("cd web3sdkio-contracts");
    await create.pressKey("enter");
