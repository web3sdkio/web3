 ...
 * @public
 */
export class Web3sdkioSDK extends RPCConnectionHandler {
  /**
  ...
  static fromSigner(
    ...
    const sdk = new Web3sdkioSDK(network || signer, options, storage);
  ...
  static fromPrivateKey(
    ...
    return Web3sdkioSDK.fromSigner(signer, network, options, storage);
  }
  ...
  /**
   * @internal
   * should never be accessed directly, use {@link Web3sdkioSDK.getPublisher} instead
   */
  private _publisher: ContractPublisher;
  /**
   * Internal handler for uploading and downloading files
   */
  private storageHandler: Web3sdkioStorage;
  /**
   ...
   */
  public storage: Web3sdkioStorage;
  /**
  ...
  public async getContractList(walletAddress: string) {
    ...
    return addressesWithContractTypes
      ...
      .map(({ address, contractType, metadata }) => {
        invariant(metadata, "All Web3sdkioContracts require metadata");
        return {
          address,
          contractType,
