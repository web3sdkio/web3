...
import DeprecatedAbi from "@web3sdkio/contracts-js/dist/abis/IDelayedRevealDeprecated.json";
import { TokensLazyMintedEvent } from "@web3sdkio/contracts-js/dist/declarations/src/DropERC721";
...
> {
  ...
  private async isLegacyContract(): Promise<boolean> {
    if (
      hasFunction<IWeb3sdkioContract>("contractVersion", this.contractWrapper)
    ) {
      try {
        const version =
