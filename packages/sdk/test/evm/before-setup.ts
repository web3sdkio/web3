...
export const mochaHooks = {
  beforeAll: async () => {
    ...
    const web3sdkioFactoryDeployer = (await new ethers.ContractFactory(
    ...
    const deployTxFactory = web3sdkioFactoryDeployer.deployTransaction;
    await deployTxFactory.wait();
    const web3sdkioRegistryAddress = await web3sdkioFactoryDeployer.registry();
    registryAddress = web3sdkioFactoryDeployer.address;

    await registryContract.grantRole(
      await registryContract.OPERATOR_ROLE(),
      web3sdkioFactoryDeployer.address,
    );
    ...
    for (const contractType in CONTRACTS_MAP) {
      ...
      for (const factory of factories) {
        ...
        const tx = await web3sdkioFactoryDeployer.addImplementation(
    ...
    process.env.registryAddress = web3sdkioRegistryAddress;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
    process.env.factoryAddress = web3sdkioFactoryDeployer.address;
    // eslint-disable-next-line turbo/no-undeclared-env-vars
    process.env.contractPublisherAddress = contractPublisher.address;

    storage = MockStorage();
    sdk = new Web3sdkioSDK(
      signer,
      {
        gasSettings: {
