...
describe("Wallet Authentication", async () => {
  ...
  it("Should reject payload with incorrect domain", async () => {
    ...
    try {
      ...
      expect(err.message).to.equal(
        "Expected domain 'test.web3sdk.io' does not match domain on payload 'web3sdk.io'",
      );
    }
  });
  ...
  it("Should reject token with incorrect domain", async () => {
    ...
    try {
      await sdk.auth.authenticate("test.web3sdk.io", token);
      expect.fail();
    } catch (err) {
      expect(err.message).to.contain(
        "Expected token to be for the domain 'test.web3sdk.io', but found token with domain 'web3sdk.io'",
      );
    }
  });
