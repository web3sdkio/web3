 ...
 *
 * @example
 * Before using this hook, you first need to set up the magic configuration in your `Web3sdkioProvider`, including your magic API key.
 *
 * ```javascript
 ...
 * ]
 *
 * // Add the above to the walletConnectors prop of your <Web3sdkioProvider />
...
export function useMagic() {
  ...
  invariant(
    wagmiContext,
    `useMagic() can only be used inside <Web3sdkioProvider />. If you are using <Web3sdkioSDKProvider /> you will have to use your own wallet-connection logic.`,
  );
  ...
  invariant(
    connector,
    "Magic connector not found, please make sure it is provided to your <Web3sdkioProvider />",
  );
